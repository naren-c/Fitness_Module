'''
problem statement:
A fitness module using tkinter,matplotlib that aims to get values of:
bmi,calories burnt,bp,pulse,sugar,heartbeat
along with a graph for:
bmi(bar graph), bloodpressure(heatmap)
using a GUI
'''
...
import numpy as np #for creating arrays used in graph
...
def bmi_plotgraph(bmi):
    y=[2400,5000,9000,12500,14000,12900,11000,8700,7400,6500,5200,4800,3750,3200,2800,2350,2000,1700,1450,1250,1150,1000,850]#number of patients
    x=np.arange(len(y))
    x_bmi = x+18
...
def blood_pressure_heatmap(systolic,diastolic):
    ...
    if systolic<90 and diastolic<60:
        text=plt.text(systolic,diastolic,("low blood pressure,\n<-- you are here"))

    elif (systolic>=90 and systolic<120) or (diastolic>=60 and diastolic<80):
        plt.text(systolic,diastolic,("blood pressure levels are normal,\n<-- you are here"))

    elif (systolic>=120 and systolic<140) or (diastolic>=80 and diastolic<90):
        plt.text(systolic,diastolic,("in prehypertension state,\n<-- you are here"))

    elif (systolic>=140 and systolic<160) or (diastolic>=90 and diastolic<100):
        plt.text(systolic,diastolic,("diagnosed with hypertension, stage 1,\n<-- you are here"))

    elif systolic>=160 or diastolic>=100:
        plt.text(systolic,diastolic,("diagnosed with hypertension, stage 2,\n<-- you are here"))
